...
assign timer_ctrl_next = apply_wstrb({31'b0, timer_irq_en_r}, dmem_wdata, dmem_wstrb);

assign dmem_write_en   = |dmem_wstrb;
assign dmem_aligned_addr = {dmem_addr[31:2], 2'b00};
assign imem_hit        = (imem_addr >= ROM_BASE) && (imem_addr <= (ROM_BASE + ROM_BYTES - 4));
assign rom_read_hit    = (dmem_aligned_addr >= ROM_BASE) && (dmem_aligned_addr <= (ROM_BASE + ROM_BYTES - 4));
assign ram_read_hit    = (dmem_aligned_addr >= RAM_BASE) && (dmem_aligned_addr <= (RAM_BASE + RAM_BYTES - 4));
assign ram_write_hit   = (dmem_addr >= RAM_BASE) && (dmem_addr < (RAM_BASE + RAM_BYTES));
assign rom_read_offset = dmem_aligned_addr - ROM_BASE;
assign ram_read_offset = dmem_aligned_addr - RAM_BASE;
assign ram_write_offset = dmem_addr - RAM_BASE;
...
assign imem_rdata = imem_hit ? {
    rom_mem[imem_addr + 32'd3],
    rom_mem[imem_addr + 32'd2],
    rom_mem[imem_addr + 32'd1],
    rom_mem[imem_addr + 32'd0]
} : 32'h0000_0013;

assign dmem_rdata =
    rom_read_hit                        ? rom_read_data :
    ram_read_hit                        ? ram_read_data :
    (dmem_aligned_addr == DONE_ADDR)    ? done_value_r :
    (dmem_aligned_addr == TIMER_VALUE_LO) ? timer_value_r[31:0] :
    (dmem_aligned_addr == TIMER_VALUE_HI) ? timer_value_r[63:32] :
    (dmem_aligned_addr == TIMER_CMP_LO) ? timer_cmp_r[31:0] :
    (dmem_aligned_addr == TIMER_CMP_HI) ? timer_cmp_r[63:32] :
    (dmem_aligned_addr == TIMER_CTRL_ADDR) ? {30'b0, timer_irq, timer_irq_en_r} :
    32'h0000_0000;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (ram_write_hit && dmem_write_en) begin
            if (dmem_wstrb[0]) ram_mem[ram_write_offset + 32'd0] <= dmem_wdata[7:0];
            if (dmem_wstrb[1] && (ram_write_offset + 32'd1 < RAM_BYTES)) ram_mem[ram_write_offset + 32'd1] <= dmem_wdata[15:8];
            if (dmem_wstrb[2] && (ram_write_offset + 32'd2 < RAM_BYTES)) ram_mem[ram_write_offset + 32'd2] <= dmem_wdata[23:16];
            if (dmem_wstrb[3] && (ram_write_offset + 32'd3 < RAM_BYTES)) ram_mem[ram_write_offset + 32'd3] <= dmem_wdata[31:24];
        end else if (dmem_write_en) begin
            case (dmem_aligned_addr)
                UART_TX_ADDR: begin
                    uart_tx_valid <= 1'b1;
                    if (dmem_wstrb[0]) begin
                        uart_tx_data <= dmem_wdata[7:0];
                    end else if (dmem_wstrb[1]) begin
                        uart_tx_data <= dmem_wdata[15:8];
                    end else if (dmem_wstrb[2]) begin
                        uart_tx_data <= dmem_wdata[23:16];
                    end else begin
                        uart_tx_data <= dmem_wdata[31:24];
                    end
                end

                DONE_ADDR: begin
                    done_value_r <= apply_wstrb(done_value_r, dmem_wdata, dmem_wstrb);
                end

                TIMER_CMP_LO: begin
                    timer_cmp_r[31:0] <= apply_wstrb(timer_cmp_r[31:0], dmem_wdata, dmem_wstrb);
                end

                TIMER_CMP_HI: begin
                    timer_cmp_r[63:32] <= apply_wstrb(timer_cmp_r[63:32], dmem_wdata, dmem_wstrb);
                end

                TIMER_CTRL_ADDR: begin
                    timer_irq_en_r <= timer_ctrl_next[0];
                    if (dmem_wstrb[0] && dmem_wdata[1]) begin
                        timer_value_r <= 64'h0000_0000_0000_0000;
                    end
                end
