...
assign csr_mip_value = timer_irq ? `YH_rv_cpu_MIP_MTIP : 32'h0000_0000;
assign ex_trap_cause =
    ex_interrupt_valid ? `YH_rv_cpu_TRAP_MTIME_INTERRUPT :
    (id_ex_ecall_r) ? `YH_rv_cpu_TRAP_ECALL_MMODE :
    (id_ex_ebreak_r) ? `YH_rv_cpu_TRAP_BREAKPOINT :
    (!csr_read_valid_ex && id_ex_csr_valid_r) ? `YH_rv_cpu_TRAP_ILLEGAL_INSN :
    (id_ex_illegal_r) ? `YH_rv_cpu_TRAP_ILLEGAL_INSN :
    (id_ex_load_r) ? `YH_rv_cpu_TRAP_LOAD_MISALIGNED :
    `YH_rv_cpu_TRAP_STORE_MISALIGNED;
...
assign ex_interrupt_valid =
    ...
    !id_ex_ebreak_r &&
    !id_ex_mret_r &&
    (csr_mstatus_r & `YH_rv_cpu_MSTATUS_MIE) != 32'h0000_0000 &&
    (csr_mie_r & `YH_rv_cpu_MIE_MTIE) != 32'h0000_0000 &&
    timer_irq;
...
YH_rv_cpu_mem_stage u_mem_stage (
    .load          (ex_mem_load_r),
