...
set LOG_FILE=%LOG_DIR%\branch_not_taken_load_independent_fold.log
...
call "%SCRIPT_DIR%prepare_xsim_runtime.bat" branch_not_taken_load_independent_fold XSIM_RUN_DIR
...
call %XVLOG% --sv -i "%PROJECT_DIR%\rtl" ^
    "%PROJECT_DIR%\tb\YH_rv_cpu_branch_not_taken_load_independent_fold_tb.v" ^
...
call %XELAB% YH_rv_cpu_branch_not_taken_load_independent_fold_tb -s YH_rv_cpu_branch_not_taken_load_independent_fold_tb_snapshot
if errorlevel 1 goto :fail

call %XSIM% YH_rv_cpu_branch_not_taken_load_independent_fold_tb_snapshot -runall -onerror quit > "%LOG_FILE%" 2>&1
type "%LOG_FILE%"
