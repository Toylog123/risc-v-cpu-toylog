...
set LOG_FILE=%LOG_DIR%\sync_dmem_preissue_diag.log
...
call "%SCRIPT_DIR%prepare_xsim_runtime.bat" sync_dmem_preissue_diag XSIM_RUN_DIR
...
call %XVLOG% --sv -i "%PROJECT_DIR%\rtl" ^
    "%PROJECT_DIR%\tb\YH_rv_cpu_sync_dmem_preissue_tb.v" ^
...
call %XELAB% YH_rv_cpu_sync_dmem_preissue_tb -s YH_rv_cpu_sync_dmem_preissue_tb_snapshot
if errorlevel 1 goto :fail

call %XSIM% YH_rv_cpu_sync_dmem_preissue_tb_snapshot !PLUSARGS! -autoloadwcfg -runall -onerror quit > "%LOG_FILE%" 2>&1
type "%LOG_FILE%"
