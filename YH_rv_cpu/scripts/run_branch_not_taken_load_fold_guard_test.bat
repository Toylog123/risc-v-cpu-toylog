...
set LOG_FILE=%LOG_DIR%\branch_not_taken_load_fold_guard.log
...
call "%SCRIPT_DIR%prepare_xsim_runtime.bat" branch_not_taken_load_fold_guard XSIM_RUN_DIR
...
call %XVLOG% --sv -i "%PROJECT_DIR%\rtl" ^
    "%PROJECT_DIR%\tb\YH_rv_cpu_branch_not_taken_load_fold_guard_tb.v" ^
...
call %XELAB% YH_rv_cpu_branch_not_taken_load_fold_guard_tb -s YH_rv_cpu_branch_not_taken_load_fold_guard_tb_snapshot
if errorlevel 1 goto :fail

call %XSIM% YH_rv_cpu_branch_not_taken_load_fold_guard_tb_snapshot -runall -onerror quit > "%LOG_FILE%" 2>&1
type "%LOG_FILE%"
