...
本项目实现了两项性能优化，满足赛题"至少完成 2 项性能优化"的要求：

#### 性能优化一：100MHz 时序收敛优化

**优化目标**：使 FPGA 能在 100MHz 频率下稳定运行

**优化措施**：
| 措施 | 说明 | 效果 |
|------|------|------|
| `-retiming` | 综合阶段允许工具自动跨寄存器移动逻辑 | 平衡组合逻辑延迟 |
| `-fanout_limit 32` | 限制单个信号的扇出数量 | 减少布线拥堵 |
| `Explore` directive | place 和 route 阶段使用探索性算法 | 更好的布局选择 |

**优化效果**：
| 指标 | 优化前 | 优化后 | 提升 |
|------|--------|--------|------|
| Setup WNS | +0.103ns | +2.481ns | 24倍 |
| Hold WHS | - | +0.602ns | - |

**优化原理**：
- **Retiming**：通过在组合逻辑路径上自动插入/移动寄存器，平衡关键路径延迟
- **Fanout Limit**：控制高扇出信号的驱动负载，提高信号完整性
- **Explore**：在布局阶段尝试更多布局方案，选择最优解

#### 性能优化二：五级流水线架构优化

**优化目标**：提高 CPU 指令吞吐率，实现 CPI 接近 1

**优化措施**：
| 措施 | 说明 | 效果 |
|------|------|------|
| 五级流水 | IF/ID/EX/MEM/WB 分离 | 每周期发射一条指令 |
| 数据转发 | EX/MEM/WB 阶段结果前递 | 消除数据冒险导致的停顿 |
| Hazard 检测 | 转发 + stall 机制 | 处理结构冒险和数据冒险 |
| 分支预测 | 静态分支预测 (默认不跳转) | 减少分支冒险 |

**优化效果**：
| 指标 | 数值 | 说明 |
|------|------|------|
| CPI | ~1.1 | 接近理想值 |
| 最高频率 | 100MHz+ | 支持 100MHz 时钟 |
| 资源利用率 | 高 | 五级流水并行工作 |

**优化原理**：
- **流水线并行**：将指令执行分为 5 个阶段，各阶段并行工作
- **数据转发**：将执行结果直接转发到需要的前级，无需等待写回
- **Hazard 单元**：检测并处理数据冒险和控制冒险
...
| CoreMark | RV32I, full workload 10次迭代 | `CoreMark/MHz = 0.912472` (host-parsed) |
| SoC 烟测 | RV32I | 通过 |
| Trap 烟测 | RV32I | 通过 |
