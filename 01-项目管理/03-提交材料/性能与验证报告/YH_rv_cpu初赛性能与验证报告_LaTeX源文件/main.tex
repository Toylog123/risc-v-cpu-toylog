...
\newcommand{\BoardLUTPct}{9.27\%}
\newcommand{\BoardFF}{2327}
\newcommand{\BoardFFPct}{2.19\%}
\newcommand{\BoardBRAM}{4}
\newcommand{\BoardBRAMPct}{2.86\%}
...
板级证据以 bitstream 下载日志、资源报告、时序报告和板卡配置记录为主。PYNQ-Z2 的 Micro-USB 串口连接到 PS 侧，不直接连接 PL 软核 UART；软核 UART 已约束到 Pmod B，可通过外接 \Metric{3.3 V} USB-UART 模块采集启动打印。在未接入外部 USB-UART 的条件下，本报告不将串口文本日志列为证据项，而以 JTAG 下载、实现报告和仿真日志支撑板级可运行性。
