...
// File role: tb/YH_rv_cpu_load_use_fast_tb.v is part of the simulation testbench and benchmark verification source.
...
// File: tb/YH_rv_cpu_load_use_fast_tb.v
...
module YH_rv_cpu_load_use_fast_tb;
...
always @(posedge clk) begin
    if (rst_n) begin
        ...
        if (debug_trace && (cycle < 80)) begin
            $display(
                "TRACE cycle=%0d pc=%h stall=%0d mem_wait=%0d dmem_req=%0d dmem_rvalid=%0d if_id_pc=%h id_ex_pc=%h ex_mem_pc=%h mem_wb_pc=%h x2=%h x3=%h x4=%h",
                cycle,
                debug_pc,
        ...
        if ((cycle > 10) &&
            ...
            if (require_no_stall_decode && (stall_decode_cycles != 0)) begin
                $fatal(1,
                    "FAIL: require_no_stall_decode set but observed stall_decode_cycles=%0d",
                    stall_decode_cycles);
            end

            $display(
                "PASS: load-use fast diagnostic completed at PC=%h cycles=%0d stall_decode_cycles=%0d require_no_stall_decode=%0d",
                debug_pc,
                cycle,
                stall_decode_cycles,
                require_no_stall_decode);
            $finish;
        end

        if (cycle > timeout_cycles) begin
            $fatal(1,
                "FAIL: timeout at PC=%h cycle=%0d stall_decode_cycles=%0d x2=%h x3=%h x4=%h",
                debug_pc,
                cycle,
                stall_decode_cycles,
                dut.u_cpu.u_regfile.regs[2],
...
initial begin
    ...
    if ($test$plusargs("dump_vcd")) begin
        $dumpfile("YH_rv_cpu_load_use_fast_tb.vcd");
        $dumpvars(0, YH_rv_cpu_load_use_fast_tb);
    end
