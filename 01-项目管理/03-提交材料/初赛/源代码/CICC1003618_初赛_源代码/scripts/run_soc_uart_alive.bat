...
call "%~dp0prepare_xsim_runtime.bat" soc_uart_alive XSIM_RUN_DIR
if not defined XSIM_RUN_DIR exit /b 1

if not exist "%XSIM_RUN_DIR%\build\sw" mkdir "%XSIM_RUN_DIR%\build\sw"
copy /y "%PROJECT_DIR%\build\sw\YH_rv_cpu_uart_alive.hex" "%XSIM_RUN_DIR%\build\sw\YH_rv_cpu_uart_alive.hex" >nul
if errorlevel 1 exit /b 1
copy /y "%PROJECT_DIR%\build\sw\YH_rv_cpu_uart_alive.mem32.hex" "%XSIM_RUN_DIR%\build\sw\YH_rv_cpu_uart_alive.mem32.hex" >nul
if errorlevel 1 exit /b 1

pushd "%XSIM_RUN_DIR%"

call %XVLOG% --sv -i "%PROJECT_DIR%\rtl" ^
    "%PROJECT_DIR%\tb\YH_rv_cpu_soc_uart_alive_tb.v" ^
...
call %XELAB% YH_rv_cpu_soc_uart_alive_tb -s YH_rv_cpu_soc_uart_alive_tb_snapshot
if errorlevel 1 goto :fail

call %XSIM% YH_rv_cpu_soc_uart_alive_tb_snapshot -runall
