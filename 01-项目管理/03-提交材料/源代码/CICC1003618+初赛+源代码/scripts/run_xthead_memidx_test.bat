...
@REM File role: scripts/run_xthead_memidx_test.bat is part of the reproducible build, simulation or reporting script.
...
@REM File: scripts/run_xthead_memidx_test.bat
...
set LOG_FILE=%LOG_DIR%\xthead_memidx_test.log
...
call "%SCRIPT_DIR%prepare_xsim_runtime.bat" xthead_memidx XSIM_RUN_DIR
...
call %XVLOG% --sv -i "%PROJECT_DIR%\rtl" ^
    "%PROJECT_DIR%\tb\YH_rv_cpu_xthead_memidx_tb.v" ^
...
call %XELAB% YH_rv_cpu_xthead_memidx_tb -s YH_rv_cpu_xthead_memidx_tb_snapshot
if errorlevel 1 goto :fail

call %XSIM% YH_rv_cpu_xthead_memidx_tb_snapshot -autoloadwcfg -runall -onerror quit > "%LOG_FILE%" 2>&1
type "%LOG_FILE%"
