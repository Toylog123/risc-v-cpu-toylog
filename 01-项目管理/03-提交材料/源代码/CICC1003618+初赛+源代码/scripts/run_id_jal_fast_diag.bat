...
@REM File role: scripts/run_id_jal_fast_diag.bat is part of the reproducible build, simulation or reporting script.
...
@REM File: scripts/run_id_jal_fast_diag.bat
...
set LOG_FILE=%LOG_DIR%\id_jal_fast_diag.log
...
call "%SCRIPT_DIR%prepare_xsim_runtime.bat" id_jal_fast_diag XSIM_RUN_DIR
...
call %XVLOG% --sv -i "%PROJECT_DIR%\rtl" ^
    "%PROJECT_DIR%\tb\YH_rv_cpu_id_jal_fast_tb.v" ^
...
call %XELAB% YH_rv_cpu_id_jal_fast_tb -s YH_rv_cpu_id_jal_fast_tb_snapshot
if errorlevel 1 goto :fail

call %XSIM% YH_rv_cpu_id_jal_fast_tb_snapshot !PLUSARGS! -autoloadwcfg -runall -onerror quit > "%LOG_FILE%" 2>&1
type "%LOG_FILE%"
