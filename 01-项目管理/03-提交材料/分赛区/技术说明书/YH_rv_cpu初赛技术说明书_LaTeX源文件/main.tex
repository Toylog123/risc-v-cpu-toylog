...
项目 & 结果 \\
\midrule
CPU 架构 & 支持 \Code{XLEN=32/64} 编译期参数化的五级顺序流水，包含 IF、ID、EX、MEM、WB，配套最小 SoC 与 FPGA 顶层。 \\
指令与优化 & 初赛冻结上板配置采用 RV32I、Zmmul、Zba/Zbb/Zbs 与 \Code{JAL} 早跳转；RV64 基础通路、Zbc、XThead indexed memidx、IDBR 作为参数化验证与探索路径，保留回归测试记录。 \\
...
工程实现包括：
\begin{itemize}
    \item 位宽与 ISA：RTL 通过 \Code{XLEN} 参数支持 RV32/RV64 编译期切换；初赛冻结上板配置为 \Code{RV32I + Zmmul + Zba/Zbb/Zbs}，并保留 RV64 smoke、Zbc、XThead、IDBR 参数化验证与探索路径；
...
RISC-V 开放 ISA 具有模块化扩展特征，便于处理器在性能、面积和功耗之间按应用场景做取舍\cite{riscv-unprivileged,case-riscv}。本设计遵循“基线指令正确、热点扩展增强、控制路径受限前移”的方法：以 \Code{XLEN=32/64} 参数化五级流水作为可扩展基础，以 RV32I 冻结配置完成 PYNQ-Z2 上板与性能评测，以 Zmmul 和 Bitmanip 子扩展覆盖 CoreMark、Dhrystone 等整型程序中的乘法、地址生成、位操作与条件选择热点\cite{riscv-bitmanip}，同时避免引入会显著推高 LUT 与时序压力的复杂乱序结构。
...
\begin{itemize}
    \item \textbf{RV32/RV64 位宽参数化：}CPU 顶层通过 \Code{XLEN} 统一控制寄存器堆、ALU、CSR、load/store 数据通路与写字节使能宽度，支持 RV32 与 RV64 在编译期配置切换；初赛冻结 bitstream 采用 RV32I 配置，相关 RV64 基础行为通过 \Code{YH\_rv\_cpu\_xlen64\_tb} 冒烟测试约束边界；
...
配置项 & 说明 \\
\midrule
\Code{XLEN} & 数据通路位宽参数，控制寄存器堆、ALU、CSR、访存数据总线和写字节使能；支持 \Metric{32} 位与 \Metric{64} 位编译期配置。 \\
RV64 基础通路 & 通过 \Code{YH\_rv\_cpu\_xlen64\_tb} 覆盖 \Code{ADDIW}、\Code{ADDW}、\Code{LD/SD}、\Code{LWU} 等基础冒烟场景，用于验证位宽参数化边界。 \\
冻结上板配置 & PYNQ-Z2 初赛 bitstream 采用 \Code{XLEN=32}，即 \Code{RV32I + Zmmul + Zba/Zbb/Zbs + JAL early redirect}，该配置对应本文性能、资源、时序与 UART 输出数据。 \\
扩展参数 & 通过 Zmmul、Bitmanip、ID branch forward 等显式开关控制可选功能，用于在性能、面积和时序之间快速筛选。 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}
    \item 位宽和功能参数集中在顶层和 SoC 边界传递，例如 \Code{XLEN}、\Code{RESET\_VECTOR} 以及 Zmmul、Bitmanip、ID branch forward 等可选开关。
...
XLEN64 smoke & \Code{XLEN=64} 下覆盖 \Code{ADDIW/ADDW}、\Code{LD/SD}、\Code{LWU} 与 64 位数据通路基础行为。 & PASS \\
...
指令覆盖说明 & 覆盖 RV32I 基础指令、Zmmul、Zba/Zbb/Zbs 及非法/非支持指令路径；\Code{XLEN=64} 基础 smoke 覆盖 RV64 位宽边界；Zbc、XThead、IDBR 探索路径保留定向回归记录。 \\
...
\ProjectName{} 形成了完整的初赛设计证据链：CPU 架构与 RTL 建模边界明确，\Code{XLEN=32/64} 位宽参数化、功能扩展开关和冻结提交配置均可追溯，功能验证与性能测试可复核，PYNQ-Z2 下载成功，资源占用与时序结果满足板级约束。该设计在保持轻量级实现和低资源占用的同时，通过热点扩展保留、前端控制流优化和参数化工程组织取得了 \Metric{\CoreMarkScore{} CoreMark/MHz} 与 \Metric{\DhrystoneDMIPSMHz{} DMIPS/MHz} 的实测表现，体现了在 FPGA 资源受限条件下对性能、面积、可扩展性与工程可实现性的综合平衡。
...
RV64 参数化冒烟测试 & \Code{源代码/CICC1003618\_初赛\_源代码/tb/YH\_rv\_cpu\_xlen64\_tb.v} 与 \Code{scripts/run\_xlen64\_smoke.bat}，用于复核 \Code{XLEN=64} 基础数据通路。 \\
\bottomrule
\end{tabularx}
