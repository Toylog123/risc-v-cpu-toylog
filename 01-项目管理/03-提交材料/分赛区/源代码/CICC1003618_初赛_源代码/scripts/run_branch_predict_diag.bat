...
@REM File role: scripts/run_branch_predict_diag.bat is part of the reproducible build, simulation or reporting script.
...
@REM File: scripts/run_branch_predict_diag.bat
...
set LOG_FILE=%LOG_DIR%\branch_predict_diag.log
set PLUSARGS=
...
call "%SCRIPT_DIR%prepare_xsim_runtime.bat" branch_predict_diag XSIM_RUN_DIR
if not defined XSIM_RUN_DIR exit /b 1
if not exist "%LOG_DIR%" mkdir "%LOG_DIR%"

for %%A in (%*) do (
    set PLUSARGS=!PLUSARGS! -testplusarg %%A
)

pushd "%XSIM_RUN_DIR%"

call %XVLOG% --sv -i "%PROJECT_DIR%\rtl" ^
    "%PROJECT_DIR%\tb\YH_rv_cpu_branch_predict_tb.v" ^
...
call %XELAB% YH_rv_cpu_branch_predict_tb -s YH_rv_cpu_branch_predict_tb_snapshot
if errorlevel 1 goto :fail

call %XSIM% YH_rv_cpu_branch_predict_tb_snapshot !PLUSARGS! -autoloadwcfg -runall -onerror quit > "%LOG_FILE%" 2>&1
type "%LOG_FILE%"
