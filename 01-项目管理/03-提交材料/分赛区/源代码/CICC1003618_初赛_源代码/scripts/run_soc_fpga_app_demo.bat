...
call "%~dp0build_firmware.bat" fpga_app_demo
...
if not defined XVLOG exit /b 1
if not defined XELAB exit /b 1
if not defined XSIM exit /b 1

call "%~dp0prepare_xsim_runtime.bat" soc_fpga_app_demo XSIM_RUN_DIR
if not defined XSIM_RUN_DIR exit /b 1

if not exist "%XSIM_RUN_DIR%\build\sw" mkdir "%XSIM_RUN_DIR%\build\sw"
copy /y "%PROJECT_DIR%\build\sw\YH_rv_cpu_fpga_app_demo.hex" "%XSIM_RUN_DIR%\build\sw\YH_rv_cpu_fpga_app_demo.hex" >nul
if errorlevel 1 exit /b 1
copy /y "%PROJECT_DIR%\build\sw\YH_rv_cpu_fpga_app_demo.mem32.hex" "%XSIM_RUN_DIR%\build\sw\YH_rv_cpu_fpga_app_demo.mem32.hex" >nul
if errorlevel 1 exit /b 1

pushd "%XSIM_RUN_DIR%"

call %XVLOG% --sv -i "%PROJECT_DIR%\rtl" ^
    "%PROJECT_DIR%\tb\YH_rv_cpu_soc_fpga_app_demo_tb.v" ^
    "%PROJECT_DIR%\fpga\vivado\src\YH_rv_uart_tx.v" ^
...
call %XELAB% YH_rv_cpu_soc_fpga_app_demo_tb -s YH_rv_cpu_soc_fpga_app_demo_tb_snapshot
if errorlevel 1 goto :fail

call %XSIM% YH_rv_cpu_soc_fpga_app_demo_tb_snapshot -runall
