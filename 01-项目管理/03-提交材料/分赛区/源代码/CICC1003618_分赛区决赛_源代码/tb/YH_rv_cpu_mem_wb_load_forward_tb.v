...
module YH_rv_cpu_mem_wb_load_forward_tb;
...
integer mem_wb_load_hazard_cycles;
integer mem_wb_load_stall_cycles;
integer stall_decode_cycles;
reg     require_no_mem_wb_load_stall;
...
always @(posedge clk) begin
    if (rst_n) begin
        ...
        if (dut.u_hazard_unit.mem_wb_load_use_hazard) begin
            mem_wb_load_hazard_cycles <= mem_wb_load_hazard_cycles + 1;
        end
        if (dut.u_hazard_unit.mem_wb_load_use_hazard && dut.stall_decode) begin
            mem_wb_load_stall_cycles <= mem_wb_load_stall_cycles + 1;
        end
        ...
        if ((cycle > 12) &&
            ...
            (dut.u_regfile.regs[5] == 32'd2) &&
            (dut.u_regfile.regs[6] == 32'd43)) begin
            if (require_no_mem_wb_load_stall && (mem_wb_load_stall_cycles != 0)) begin
                $fatal(1,
                    "FAIL: observed MEM/WB load-use stall cycles=%0d hazard_match_cycles=%0d stall_decode_cycles=%0d",
                    mem_wb_load_stall_cycles,
                    mem_wb_load_hazard_cycles,
                    stall_decode_cycles);
            end

            $display(
                "PASS: mem/wb load forward diagnostic completed at PC=%h cycles=%0d mem_wb_load_hazard_cycles=%0d mem_wb_load_stall_cycles=%0d stall_decode_cycles=%0d require_no_mem_wb_load_stall=%0d",
                debug_pc,
                cycle,
                mem_wb_load_hazard_cycles,
                mem_wb_load_stall_cycles,
                stall_decode_cycles,
                require_no_mem_wb_load_stall);
            $finish;
        end

        if (cycle > timeout_cycles) begin
            $fatal(1,
                "FAIL: timeout at PC=%h cycle=%0d x3=%h x4=%h x5=%h x6=%h mem_wb_load_hazard_cycles=%0d",
                debug_pc,
                cycle,
...
initial begin
    ...
    cycle = 0;
    timeout_cycles = 120;
    mem_wb_load_hazard_cycles = 0;
    mem_wb_load_stall_cycles = 0;
    stall_decode_cycles = 0;
    require_no_mem_wb_load_stall = 1'b0;

    if ($test$plusargs("require_no_mem_wb_load_stall")) begin
        require_no_mem_wb_load_stall = 1'b1;
    end
    ...
    imem[3] = rv32_i(12'sd2, 5'd0, 3'b000, 5'd5, 7'b0010011); // independent
    imem[4] = rv32_i(12'sd1, 5'd3, 3'b000, 5'd6, 7'b0010011); // addi x6, x3, 1
