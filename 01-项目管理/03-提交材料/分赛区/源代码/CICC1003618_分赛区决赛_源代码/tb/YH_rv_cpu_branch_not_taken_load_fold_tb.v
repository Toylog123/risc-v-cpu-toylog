...
module YH_rv_cpu_branch_not_taken_load_fold_tb;
...
integer cycle;
integer idx;
integer not_taken_load_fold_count;
...
always @(posedge clk) begin
    if (rst_n) begin
        ...
        if (dut.id_branch_not_taken_fold_valid && dut.fold_id_load) begin
            not_taken_load_fold_count <= not_taken_load_fold_count + 1;
        end
        ...
        if ((cycle > 20) &&
            ...
            if (not_taken_load_fold_count == 0) begin
                $fatal(1, "FAIL: load fall-through was never folded; cycles=%0d", cycle);
            end
            $display(
                "PASS: not-taken load fold diagnostic cycles=%0d load_folds=%0d x2=%0d x7=%h x8=%h",
                cycle,
                not_taken_load_fold_count,
...
initial begin
    ...
    imem[3] = rv32_b(13'sd16, 5'd0, 5'd1, 3'b001, 7'b1100011); // bne x1,x0,skip (not taken)
    imem[4] = rv32_i(12'sd0, 5'd5, 3'b100, 5'd7, 7'b0000011);  // lbu x7,0(x5)
    imem[5] = rv32_i(12'sd1, 5'd7, 3'b000, 5'd8, 7'b0010011);  // addi x8,x7,1
    imem[6] = rv32_i(12'sd1, 5'd2, 3'b000, 5'd2, 7'b0010011);  // addi x2,x2,1
    imem[7] = rv32_b(-13'sd16, 5'd3, 5'd2, 3'b100, 7'b1100011); // blt x2,x3,loop
    imem[8] = rv32_j(21'sd0, 5'd0, 7'b1101111); // park
