...
module YH_rv_cpu_branch_not_taken_load_use_guard_tb;
...
always @(posedge clk) begin
    if (rst_n) begin
        ...
        if (dut.id_branch_not_taken_fold_valid && dut.fold_id_load) begin
            $fatal(1, "FAIL: load fold used with immediate load-use at PC=%h cycle=%0d", debug_pc, cycle);
        end
        ...
        if ((cycle > 20) &&
            ...
            $display("PASS: not-taken load-use guard cycles=%0d", cycle);
            $finish;
        end

        if (cycle > 220) begin
            $fatal(1,
                "FAIL: timeout at PC=%h cycle=%0d x2=%0d x7=%h x8=%h",
                debug_pc,
                cycle,
...
initial begin
    ...
    imem[4] = rv32_b(13'sd16, 5'd0, 5'd1, 3'b001, 7'b1100011);
    imem[5] = rv32_i(12'sd0, 5'd5, 3'b100, 5'd7, 7'b0000011);
    imem[6] = rv32_i(12'sd1, 5'd7, 3'b000, 5'd8, 7'b0010011);
    imem[7] = rv32_i(12'sd1, 5'd2, 3'b000, 5'd2, 7'b0010011);
