...
set LOG_FILE=%PROJECT_DIR%\build\sw\YH_rv_cpu_perf_demo.log
set XVLOG=
set XELAB=
set XSIM=
set XSIM_RUN_DIR=
...
call "%~dp0prepare_xsim_runtime.bat" perf_demo XSIM_RUN_DIR
...
call %XVLOG% --sv -i "%PROJECT_DIR%\rtl" ^
    "%PROJECT_DIR%\tb\YH_rv_cpu_perf_demo_tb.v" ^
...
call %XELAB% YH_rv_cpu_perf_demo_tb -s YH_rv_cpu_perf_demo_tb_snapshot
if errorlevel 1 goto :fail

call %XSIM% YH_rv_cpu_perf_demo_tb_snapshot -testplusarg "max_cycles=%MAX_CYCLES%" -runall > "%LOG_FILE%" 2>&1
