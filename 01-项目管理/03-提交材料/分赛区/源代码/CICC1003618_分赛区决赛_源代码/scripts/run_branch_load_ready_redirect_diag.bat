...
set LOG_FILE=%LOG_DIR%\branch_load_ready_redirect_diag.log
...
call "%SCRIPT_DIR%prepare_xsim_runtime.bat" branch_load_ready_redirect_diag XSIM_RUN_DIR
...
call %XVLOG% --sv -i "%PROJECT_DIR%\rtl" ^
    "%PROJECT_DIR%\tb\YH_rv_cpu_branch_load_ready_redirect_tb.v" ^
...
call %XELAB% YH_rv_cpu_branch_load_ready_redirect_tb -s YH_rv_cpu_branch_load_ready_redirect_tb_snapshot
if errorlevel 1 goto :fail

call %XSIM% YH_rv_cpu_branch_load_ready_redirect_tb_snapshot !PLUSARGS! -autoloadwcfg -runall -onerror quit > "%LOG_FILE%" 2>&1
type "%LOG_FILE%"
