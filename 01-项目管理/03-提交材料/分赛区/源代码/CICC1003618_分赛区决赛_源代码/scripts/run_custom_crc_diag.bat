...
set LOG_FILE=%LOG_DIR%\custom_crc_diag.log
...
call "%SCRIPT_DIR%prepare_xsim_runtime.bat" custom_crc_diag XSIM_RUN_DIR
...
call %XVLOG% --sv -i "%PROJECT_DIR%\rtl" ^
    "%PROJECT_DIR%\tb\YH_rv_cpu_custom_crc_tb.v" ^
...
call %XELAB% YH_rv_cpu_custom_crc_tb -s YH_rv_cpu_custom_crc_tb_snapshot
if errorlevel 1 goto :fail

call %XSIM% YH_rv_cpu_custom_crc_tb_snapshot -autoloadwcfg -runall -onerror quit > "%LOG_FILE%" 2>&1
type "%LOG_FILE%"
findstr /c:"FAIL:" "%LOG_FILE%" >nul
if not errorlevel 1 goto :fail
findstr /c:"PASS: custom crc diagnostic completed" "%LOG_FILE%" >nul
if errorlevel 1 goto :fail
set RUN_STATUS=0
goto :done
