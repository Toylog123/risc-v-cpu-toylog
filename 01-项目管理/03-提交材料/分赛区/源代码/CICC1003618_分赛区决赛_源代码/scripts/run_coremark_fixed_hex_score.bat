...
if "%TEST_TOP%"=="" set TEST_TOP=YH_rv_cpu_coremark_rv32_zmmul_bitmanip_zicond_xthead_idbr_tb
if "%ROM_HEX%"=="" (
    echo usage: run_coremark_fixed_hex_score.bat ^<test_top^> ^<rom_hex^> [timer_hz] [max_cycles] [summary_file]
    exit /b 2
)
if "%TIMER_HZ%"=="" set TIMER_HZ=100000000UL
if "%MAX_CYCLES%"=="" set MAX_CYCLES=400000000
...
call "%~dp0prepare_xsim_runtime.bat" coremark_fixed_hex_score XSIM_RUN_DIR
if not defined XSIM_RUN_DIR exit /b 1

if not exist "%XSIM_RUN_DIR%\build\sw" mkdir "%XSIM_RUN_DIR%\build\sw"
copy /y "%ROM_HEX%" "%XSIM_RUN_DIR%\build\sw\YH_rv_cpu_coremark_rv32.hex" >nul
...
call %XSIM% %TEST_TOP%_fixed_hex_snapshot -testplusarg "max_cycles=%MAX_CYCLES%" -runall > "%LOG_FILE%" 2>&1
type "%LOG_FILE%"
