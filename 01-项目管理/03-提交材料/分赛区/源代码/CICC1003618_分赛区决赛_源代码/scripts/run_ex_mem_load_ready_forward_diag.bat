...
set LOG_FILE=%LOG_DIR%\ex_mem_load_ready_forward_diag.log
...
call "%SCRIPT_DIR%prepare_xsim_runtime.bat" ex_mem_load_ready_forward_diag XSIM_RUN_DIR
...
call %XVLOG% --sv -i "%PROJECT_DIR%\rtl" ^
    "%PROJECT_DIR%\tb\YH_rv_cpu_ex_mem_load_ready_forward_tb.v" ^
...
call %XELAB% YH_rv_cpu_ex_mem_load_ready_forward_tb -s YH_rv_cpu_ex_mem_load_ready_forward_tb_snapshot
if errorlevel 1 goto :fail

call %XSIM% YH_rv_cpu_ex_mem_load_ready_forward_tb_snapshot !PLUSARGS! -autoloadwcfg -runall -onerror quit > "%LOG_FILE%" 2>&1
type "%LOG_FILE%"
