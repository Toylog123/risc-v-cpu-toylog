...
\newcommand{\BoardLUT}{4934}
\newcommand{\BoardFF}{2327}
\newcommand{\BoardBRAM}{4}
\newcommand{\BoardDSP}{15}
\newcommand{\BoardWNS}{+0.440\,ns}
\newcommand{\BoardWHS}{+0.151\,ns}
\newcommand{\CoverLogoPath}{./logo.png}
\newcommand{\BitstreamName}{YH\_rv\_cpu\_pynq\_z2\_sysclk\_8p000ns\_cpu50\_zmmul\_bitmanip\_noidbr\_20260430.bit}
...
\IfFileExists{figures/#1}{
    \includegraphics[width=0.94\textwidth]{figures/#1}
}{
...
指标 & 结果 \\
\midrule
Slice LUTs & \Metric{\BoardLUT{} / 53200 = 9.27\%} \\
Slice Registers & \Metric{\BoardFF{} / 106400 = 2.19\%} \\
Block RAM Tile & \Metric{\BoardBRAM{} / 140 = 2.86\%} \\
...
\end{itemize}
板级验证证据以 bitstream 下载日志、器件识别、实现报告、仿真回归结果和基准测试摘要为主。PYNQ-Z2 的 Micro-USB 串口连接至 PS 端，不直接连接 PL 软核 UART；软核 UART 已约束到 Pmod B，可通过外接 \Metric{3.3 V} USB-UART 采集文本日志。未接入外部 USB-UART 时，材料以 JTAG 下载成功、资源时序报告和脚本化验证日志作为板级证据。
