...
## 2026-03-17 追加：dmem BRAM 推断成功后的资源口径

- 这一轮已经把 `dmem` 从同步语义推进到真正的 BRAM 实现。
- 最新综合结果应以这版为准：
- `50MHz`
  - `Slice LUTs = 2590`
  - `Slice Registers = 2033`
  - `Block RAM Tile = 2`
  - `DSP = 0`
  - `WNS = 7.556ns`
- `100MHz`
  ...
  - `WNS = -2.470ns`
- 这次结果对板卡申请和会前汇报的意义：
  - `Nexys A7-100T` 这类板卡资源依然明显够用
  - 资源压力比之前更小了，申请理由不再是“担心装不下”，而是“需要真实板级闭环”
  - 比赛要求的 `50MHz` 更稳
  - 当前仍应明确说明：`100MHz` 继续优化、`imem/ROM` 继续优化
